...
type LocationType = 'country' | 'region' | 'district';
...
export default function Locations() {
    ...
    const { data: countriesData, isLoading: countriesLoading } = useCountries({
        search: activeTab === 'country' ? searchTerm : undefined,
    });
    ...
    const isLoading = countriesLoading || regionsLoading || districtsLoading;
    const isMutating =
    ...
    const handleOpenDialog = (item?: Country | Region | District) => {
        if (item) {
            const regionValue = 'region' in item
                ? (typeof item.region === 'string' ? item.region : item.region_detail?.id)
                : undefined;
        ...
        try {
            if (editingItem.id) {
                ...
                if (activeTab === 'country') {
                    await updateCountry.mutateAsync({ id: editingItem.id, data });
    ...
    const getTabConfig = () => {
        switch (activeTab) {
            case 'country':
                return {
                    title: 'Mamlakatlar',
                    description: 'Mamlakatlar ro\'yxati va boshqaruvi',
                    icon: Globe,
                    data: countries,
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Mamlakatlar, viloyatlar va tumanlarni boshqaring
                    </p>
                </div>
            </div>
                                                        ...
                                                        <TableCell>
                                                            {(() => {
                                                                ...
                                                                if (region && typeof region === 'object' && 'name' in region) {
                                                                    return <Badge variant="outline">{region.name}</Badge>;
                                                                }
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[525px]">
                    <form onSubmit={handleSubmit}>
                        <DialogHeader>
                            <DialogTitle>
                                {editingItem?.id ? 'Tahrirlash' : 'Qo\'shish'}
                            </DialogTitle>
                            <DialogDescription>
                                {config.title} ma'lumotlarini kiriting
                            </DialogDescription>
                        </DialogHeader>
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Textarea
                                    ...
                                    onChange={(e) =>
                                        setEditingItem((prev) => ({ ...prev!, geo_json: e.target.value }))
                                    }
                                    rows={4}
